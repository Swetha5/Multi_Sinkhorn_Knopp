...
from everything_at_once.trainer.utils import load_pickle
...
class MSRVTT_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.training == 'eval_knn':
            # for knn evaluation get first caption for train samples
        ...
        video_pooled = get_average_feature(feat_tokens=video.unsqueeze(0), feat_mask=video_mask.unsqueeze(0)).squeeze(dim=0)
        text_pooled = get_average_feature(feat_tokens=text.unsqueeze(0), feat_mask=text_mask.unsqueeze(0)).squeeze(dim=0)
