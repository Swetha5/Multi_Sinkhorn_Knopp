...
from .librosa_utils import librosa_time_to_frames
...
def create_long_video_features(feat_2d, n_tokens, strategy='clip'):
    video = torch.zeros(n_tokens, feat_2d.shape[-1])
    video_mask = torch.zeros(n_tokens)
    cur_n_tokens_2d, dim_2d = feat_2d.shape
    if cur_n_tokens_2d <= n_tokens:
        video[:cur_n_tokens_2d] = F.normalize(feat_2d, dim=1)
        video_mask[:cur_n_tokens_2d] = 1
    else:
        video[:n_tokens] = F.normalize(feat_2d[:n_tokens], dim=1)
        video_mask[:n_tokens] = 1
    return video, video_mask


def _crop_audio_from_mel_spec(start, end, mel_spec):
    frames = librosa_time_to_frames([start, end], sr=16000, hop_length=160,
                                         n_fft=400)
...
def cut_into_clips_no_audio(video, video_mask, text, text_mask, raw_text, id_, dataset,
                   n_clips):
    # create video clips
    n_video_tokens = int(video_mask.shape[0] // n_clips)
    video = video.view(n_clips, n_video_tokens, video.size(-1))
    ...
    dataset = [dataset] * n_clips
    raw_text = [raw_text] * n_clips
    return video, video_mask, text, text_mask, raw_text, id_, dataset


def get_average_feature(feat_tokens, feat_mask):
    '''
    vid_feat [10,12,4096], vid_mask [10,12]
    txt_feat [10,20,300], txt_mask [10,20]
    '''
    feat_mask = feat_mask.unsqueeze(2).expand_as(feat_tokens)
    return (feat_tokens * feat_mask).sum(1) / (feat_mask.sum(1) + 0.00001)