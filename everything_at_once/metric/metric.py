...
class RetrievalMetric:
    ...
    def __call__(self, sims_dict, complete_dataset_size=None):
        if self._task in sims_dict:
            return retrieval_metrics(sims_dict[self._task], complete_dataset_size=complete_dataset_size, metric_name=self.__name__)
        elif self._inv_task in sims_dict:
            return retrieval_metrics(sims_dict[self._inv_task].T, complete_dataset_size=complete_dataset_size, metric_name=self.__name__)
        else:
            return {}
...
def retrieval_metrics(sims, break_ties='averaging', complete_dataset_size=None, metric_name='metric'):
